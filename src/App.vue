...
const getHint = () => {
  if (remainingHints.value <= 0) {
    inputError.value = `Need ${10 - (wordCount.value % 10)
...
<template>
  <main class="min-h-screen flex items-center justify-center text-black bg-white">
    <!-- Home Page -->
    <section v-show="isHomePage">
      <div class="flex flex-col items-center justify-center text-center px-4 sm:px-6 md:px-8">
        <div class="flex flex-col items-center">
          <h1 class="text-[72px] font-bold text-[#1882FF] sm:-tracking-[-0.1em]">
            TORKUM
          </h1>
        ...
        <div class="mt-6 w-full max-w-md">
          <form @submit.prevent="handleSubmit" class="flex flex-col gap-4 items-center">
            <input v-model.trim="userName" type="text" placeholder="Please enter your name"
    ...
    <section v-show="isAchievementPage">
      <div class="flex flex-col space-y-4 w-[300px] sm:w-[600px] md:w-[700px] border rounded shadow-md p-4">
        <h1 class="text-3xl text-[#1882ff] sm:text-4xl font-bold text-center">
          Achievement🏆
        </h1>
        <div class="flex flex-col space-y-2 items-center max-h-[380px] overflow-y-scroll">
          <div v-for="item in achievement" :key="item.id"
            ...
            <div class="p-4 rounded" :class="item.isCompleted ? item.bg : 'bg-[#ffe45bde]'">
              <span class="text-4xl">{{
      ...
      <div class="flex items-center justify-center mt-4">
        <button class="btn bg-[#D9D9D9] text-[#595959] border-0" @click="backHome">
        ...
        <div class="grid w-full md:w-[700px] place-items-center py-9">
          <div class="w-full">
            ...
            <div class="gap-4 grid grid-cols-2 max-w-[80%] mx-auto mt-9">
              <button class="btn bg-[#D9D9D9] text-[#595959] border-0"
                @click="stepCounter == 0 ? backHome() : decrement()" :class="stepCounter === 2 ? 'bg-blue-500 text-white' : 'bg-[#D9D9D9]'
                  ">
                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                  width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                  <path fill-rule="evenodd"
    ...
    <!-- Game Page -->
    <section v-show="isGamePage" class="self-start flex justify-center items-center min-h-screen">
      <div class="flex flex-col space-y-4 w-[280px] sm:w-[400px] md:w-[574px]">
        <div>
            ...
            <ul class="menu sm:menu-horizontal rounded-box bg-white shadow-lg">
              ...
              <li>
                <details ref="timeDropdown">
                  <summary class="tooltip px-9" data-tip="Normal" @click="setGameMode('timer')">
                    Timer
                  </summary>
              ...
              <li>
                <details ref="wordDropdown">
                  <summary class="tooltip" data-tip="Difficult" @click="setGameMode('moreWord')">
                    More Word
                  </summary>
          ...
          <div class="flex justify-between w-full">
            <button @click="getHint" class="px-4 py-1 bg-white border rounded-md shadow-md flex items-center space-x-2"
            ...
            <div
              ...
              <span>character {{ chooseWord }}</span>
            </div>
          </div>
        </div>
        ...
        <section class="flex items-center justify-between">
          ...
          <span @click="onReset" class="mr-4 cursor-pointer">
            <svg class="w-5 sm:w-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
              <path
        ...
        <section class="flex items-center justify-center flex-col space-y-4 pb-4">
          <div :class="{
          ...
          <div class="border rounded-xl shadow-md">
            <input
              ...
              @keydown.enter="handleInputChange" v-model="wordInput" placeholder="Please enter a word here"
              type="text" />
          </div>
          <div v-show="usedWord.length > 0"
            class="border rounded-md shadow-md w-[280px] sm:w-[400px] md:w-[574px] p-4 h-[210px] overflow-auto sm:h-[300px]">
            <span class="text-md sm:text-lg font-light text-neutral-500">Used word</span>
            <div class="flex items-center justify-start flex-wrap gap-4 uppercase font-semibold text-[#777777] py-4">
              <span v-for="(value, key) in usedWord" :key="key"
                class="text-sm sm:text-md w-fit px-4 py-2 bg-[#d9d9d9] rounded-lg text-center">{{ value }}</span>
            </div>
          </div>
        </section>
      </div>
      ...
      <div class="toast toast-top toast-end lg:toast-end lg:toast-bottom">
        <div v-for="item in visibleAchievement" :key="item.id"
    ...
    <section v-show="isResultPage" class="w-[600px] p-3">
      <div class="mb-5">
        <button @click="backHome" class="px-8 py-2 border rounded-md shadow-md bg-white">
          Back
        </button>
      </div>
      <div class="border border-gray-200 shadow-lg">
        <p class="text-center text-3xl font-bold py-8">Game Over</p>
        <div class="flex flex-col items-center sm:flex-row py-3 border-b w-full">
          <div class="flex w-3/5 justify-between sm:flex-col sm:items-center sm:w-1/3">
            <p class="text-2xl">Score</p>
            <p class="text-3xl sm:text-5xl">{{ score }}</p>
          </div>
          <div class="flex w-3/5 justify-between sm:flex-col items-center sm:w-1/3">
            <p class="text-2xl">Total Time</p>
            <p class="text-3xl sm:text-5xl">{{ totalTime }}</p>
          </div>
          <div class="flex w-3/5 justify-between items-start sm:flex-col sm:items-center sm:w-1/3">
            <p class="text-2xl">Word Count</p>
            <p class="text-3xl sm:text-5xl">{{ wordCount }}</p>
          </div>
        </div>

        <div class="py-10 flex flex-col gap-3 items-center justify-center md:flex-row md:gap-8">
          <button
          ...
          <button class="w-3/5 sm:w-1/3 bg-[#1882FF] px-5 py-1.5 text-white text-xl font-bold rounded-md"
            v-show="!isShowStats" @click="showStats">
            Show stats
          </button>
          <button class="w-3/5 sm:w-1/3 bg-[#1882FF] px-5 py-1.5 text-white text-xl font-bold rounded-md"
            v-show="isShowStats" @click="showUsedWord">
            Used word
          </button>
        </div>
        <div class="bg-[#1882FF] text-2xl text-white text-center py-4 px-1">
          ...
          <div class="mt-2">
            <span v-for="word in longestWords" :key="word"
              class="bg-gray-200 px-2 py-1 text-black text-lg m-2 rounded-md">{{ word }}</span>
          </div>
        </div>
      </div>
      ...
      <div v-show="isShowStats" class="mt-8 shadow-md">
        <div class="flex justify-between">
          ...
          <div class="bg-gray-200 text-center border-2 border-r-gray-300 py-2 text-lg font-bold w-1/3">
            Score
          </div>
          <div class="bg-gray-200 text-center border=2 border-r-gray-300 py-2 text-lg font-bold w-1/3">
            Total time
        ...
        <div class="border border-gray-300">
          <div v-for="stats in allStats" class="grid grid-cols-3 grid-flow-row">
            <div v-for="information in stats" class="text-center py-2 border border-r-gray-300">
              {{ information }}
            </div>
          </div>
        </div>
